...
describe('AutocompleteItem component', () => {
  ...
  describe('autocomplete text', () => {
    ...
    it('shows additional info when set', () => {
      ...
      expect(
        wrapper.find('.files-collaborators-autocomplete-user-text .oc-text-muted').text()
      ).toEqual('some text')
    })
    it('does not shows additional info when not set', () => {
      const wrapper = createWrapper({ shareWithAdditionalInfo: undefined })
      expect(
        wrapper.find('.files-collaborators-autocomplete-user-text .oc-text-muted').exists()
      ).toBeFalsy()
    })
    it('shows the share type', () => {
      const wrapper = createWrapper({ shareType: 0 })
      expect(wrapper.find('.files-collaborators-autocomplete-user-text > div').text()).toEqual(
        'User'
      )
    })
  })
